...
> ⚠️ **这是某一时点的差距快照，正文表格不再逐条维护**（保留原样以便对照"当初缺什么"）。
> 已经收口的部分见下方「后续更新」，判断现状请以代码与 `CHANGELOG.md` 的 `[Unreleased]` 为准。

---

## 0. 后续更新（正文之外）

- **搜索/筛选类全部改为"输入即过滤"**：正文里"输入即刷""搜索"的写法（`✅ 仅包名 contains，输入即刷`
  等）描述的是当时的行为，其中"输入即刷"实际是坏的——前端对同一元素**先 action 后 bind**
  （cjxt `public/js/cangjie-ui.js:333` vs `:490`），在 `on("input")` 里刷新读到的仍是上一次的关键字，
  且渲染不读过滤信号 → **搜索完全无效，必须再点一次刷新/筛选**。现已全部改为信号派生
  （`src/ui/list_binding.cj`），覆盖包管理（搜索/组织筛选/分页）、用户管理（搜索）、日志（关键字/类型/
  结果 + 服务端分页）、团队弹窗（包关联/成员关联搜索）、发布计划新建向导的包选择。
- **发布计划包版本列表改主子表**：原「版本」按钮弹窗 → 同页**左主（包）右子（版本）**；
  单版本详情（README 全文）保留弹窗。见 `AGENTS.md`「后台：包管理（左主右子）」。
- **发布计划列表倒序**（新的在前）；详情页新增「隐藏不推送的项」开关（只筛显示行，计数仍按全量）。
- 仍然成立的部分：分类筛选、卡片式列表、Docs 站内搜索等条目未动，按需再排。
