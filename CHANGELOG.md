...
### Fixed — 空态插图变成黑块 / 版本对照卡多出一张大插图

- **空态插图整张变黑**（cjxt）：`Empty` 默认 SVG 里的 `fill="var(--el-fill-color-N)"` 引用的变量在
  vendored EP 样式里并不存在（只有 `--el-empty-fill-color-*`）；`var()` 解析失败时 `fill` 回落到
  **初始值黑色**而不是继承 —— 所有空态都成了几块黑方块，看着像图片裂了。已改成引用已定义的变量，
  并加回归断言（不得再出现未定义的 `--el-fill-color-N`）。
- **创建计划页「版本对照」卡**：说明（对照中 / 未选上游 / 「共 N 项 · 需发布 M」摘要）原来和
  **空态**共用同一个提示槽（`listBody` 用的是带插图的 `emptyState`），于是"有 5 行数据 + 一张大插图 +
  一句摘要"叠在一起。现在两者分开：说明走普通提示行，空态插图只在确实没东西可看时出现。
- 回归：`pages_plan_render_test.cj` 新增创建页 3 条（三张表表头 / 说明不算空态 / 真空态才给提示）。
