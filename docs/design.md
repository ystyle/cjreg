...
> **请求体上限与内存**：`POST /pkg` 当前把**整个请求体读入内存**后再分段落盘（峰值 ≈ 包体 2–3 倍），
> 受仓颉 GC 堆默认上限（256 MB，`cjHeapSize`）约束；请求体上限由配置 `max_request_bytes` 控制（默认 500 MiB）。
> 大包（>30 MiB）需同时调大 `cjHeapSize`；**流式解析（边读边算 SHA-256、临时文件 + 原子 rename 落盘）列入决赛迭代**，
> 见 `docs/finals-plan.md` 迭代 1。
